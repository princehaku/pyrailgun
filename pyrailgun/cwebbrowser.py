...
class CWebBrowser(object):
    ...
    def sendText(self, selector, text, keyboard_modifiers=Qt.NoModifier, wait_load=False, wait_requests=None,
                 timeout=None):
        """
    ...
    def sendKeys(self, selector, keys, keyboard_modifiers=Qt.NoModifier, wait_load=False, wait_requests=None,
                 timeout=None):
        """
    ...
    def native_click(self, selector, wait_load=False, wait_requests=None, timeout=1, offsetx=5, offsety=5, real=False,
                     pdb=False):
        """
    ...
    def native_click_ajax(self, selector, wait_requests=1, timeout=None, offsetx=0, offsety=0):
        ...
        @param offsety: offset to click on the widget to the top left of it on the Y axix (top to bottom)
        """
        return self.native_click(selector, wait_requests=wait_requests, timeout=timeout, offsetx=offsetx,
                                 offsety=offsety)
...
class ExtendedNetworkCookieJar(QNetworkCookieJar):
    ...
    def add_cookie(self, domain, name, value):
        ...
        self.setAllCookies(filter(bool, self.cookies))
        pass
